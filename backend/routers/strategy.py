...
def _base_query() -> SelectOfScalar[Strategy]:
    return (
        select(Strategy)
        .options(
            joinedload(Strategy.layer),
            joinedload(Strategy.color_layer),
            joinedload(Strategy.phase),
        )
...
def _validate_references(
    session: Session, layer_id: int, color_layer_id: int, phase_id: int
) -> None:
    if session.get(Layer, layer_id) is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Invalid layer_id",
        )
...
def update_strategy(
    *, strategy_id: int, payload: StrategyUpdate, session: SessionDep
    ...
    if "layer_id" in data or "color_layer_id" in data or "phase_id" in data:
        ...
        _validate_references(session, layer_id, color_layer_id, phase_id)
