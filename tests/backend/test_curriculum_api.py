...
def test_curriculum_filtering_and_relations(client) -> None:
    ...
    paginated = client.get("/curriculum", params={"limit": 2, "offset": 1})
...
def test_curriculum_crud(client) -> None:
    ...
    created = client.post("/curriculum", json=create_payload)
    assert created.status_code == 201
    curriculum_id = created.json()["id"]

    detail = client.get(f"/curriculum/{curriculum_id}")
    assert detail.status_code == 200
    assert detail.json()["expression"] == "Integration Testing"

    update_payload = {"expression": "Updated Expression", "dosage": "Toxic"}
    updated = client.put(f"/curriculum/{curriculum_id}", json=update_payload)
    assert updated.status_code == 200
    updated_body = updated.json()
    assert updated_body["expression"] == "Updated Expression"
    assert updated_body["dosage"] == "Toxic"

    delete_response = client.delete(f"/curriculum/{curriculum_id}")
    assert delete_response.status_code == 204

    missing = client.get(f"/curriculum/{curriculum_id}")
