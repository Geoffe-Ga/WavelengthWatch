...
def test_strategy_filtering(client) -> None:
    response = client.get(
        "/api/v1/strategy", params={"color_layer_id": 1, "phase_id": 5}
    )
...
def test_strategy_crud(client) -> None:
    ...
    created = client.post("/api/v1/strategy", json=create_payload)
    ...
    missing = client.get(f"/api/v1/strategy/{strategy_id}")
