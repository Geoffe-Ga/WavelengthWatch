# Graph Report - WavelengthWatch  (2026-07-31)

## Corpus Check
- 247 files · ~125,207 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3205 nodes · 6341 edges · 652 communities (112 shown, 540 thin omitted)
- Extraction: 79% EXTRACTED · 21% INFERRED · 0% AMBIGUOUS · INFERRED: 1319 edges (avg confidence: 0.78)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `31a59aab`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- Community 0
- Community 1
- Community 2
- Community 3
- Community 4
- Community 5
- Community 6
- Community 7
- Community 8
- Community 9
- Community 10
- Community 11
- Community 12
- Community 13
- Community 14
- Community 15
- Community 16
- Community 17
- Community 18
- Community 19
- Community 20
- Community 21
- Community 22
- Community 23
- Community 24
- Community 25
- Community 26
- Community 27
- Community 28
- Community 29
- Community 30
- Community 31
- Community 32
- Community 33
- Community 34
- Community 35
- Community 36
- Community 37
- Community 38
- Community 39
- Community 40
- Community 41
- Community 42
- Community 43
- Community 44
- Community 45
- Community 46
- Community 47
- Community 48
- Community 49
- Community 50
- Community 51
- Community 52
- Community 53
- Community 54
- Community 55
- Community 56
- Community 57
- Community 58
- Community 59
- Community 60
- Community 61
- Community 62
- Community 63
- Community 64
- Community 65
- Community 66
- Community 67
- Community 68
- Community 69
- Community 70
- Community 71
- Community 72
- Community 73
- Community 74
- Community 75
- Community 76
- Community 77
- Community 78
- Community 79
- Community 80
- Community 81
- Community 82
- Community 83
- Community 84
- Community 85
- Community 86
- Community 87
- Community 88
- Community 89
- Community 90
- Community 91
- Community 92
- Community 93
- Community 94
- Community 95
- Community 96
- Community 97
- Community 98
- Community 99
- Community 100
- Community 101
- Community 102
- Community 103
- Community 104
- Community 105
- Community 106
- Community 107
- Community 108
- Community 109
- Community 110
- Community 111
- Community 112
- Community 113
- Community 114
- Community 115
- Community 116
- Community 117
- Community 118
- Community 119
- Community 120
- Community 121
- Community 122
- Community 123
- Community 124
- Community 126
- Community 127
- Community 128
- Community 129
- Community 130
- Community 131
- Community 132
- Community 133
- Community 134
- Community 135
- Community 136
- Community 137
- Community 138
- Community 139
- Community 140
- Community 141
- Community 142
- Community 143
- Community 144
- Community 145
- Community 146
- Community 147
- Community 148
- Community 149
- Community 150
- Community 151
- Community 152
- Community 153
- Community 154
- Community 155
- Community 156
- Community 157
- Community 158
- Community 159
- Community 160
- Community 161
- Community 162
- Community 163
- Community 164
- Community 165
- Community 166
- Community 167
- Community 168
- Community 169
- Community 170
- Community 171
- Community 172
- Community 173
- Community 174
- Community 175
- Community 176
- Community 177
- Community 178
- Community 179
- Community 180
- Community 181
- Community 182
- test_analytics_overview_longest_streak_calculation
- test_analytics_overview_longest_streak_equals_current
- test_analytics_overview_longest_streak_ge_current
- test_analytics_overview_current_streak_calculation
- test_analytics_overview_longest_streak_zero_for_no_entries
- test_emotional_landscape_basic_structure
- test_emotional_landscape_layer_distribution
- test_emotional_landscape_phase_distribution
- test_emotional_landscape_top_emotions
- test_emotional_landscape_empty_user
- test_emotional_landscape_date_filtering
- test_analytics_overview_current_streak_with_gap
- test_emotional_landscape_default_dates
- test_self_care_analytics_basic_structure
- test_self_care_top_strategies
- FastAPI
- Any
- Session
- Any
- SQLModel
- AnalyticsOverview
- datetime
- EmotionalLandscape
- ge
- GrowthIndicators
- le
- Query
- SelfCareAnalytics
- Session
- SessionDep
- TemporalPatterns
- Response
- SessionDep
- ge
- le
- Query
- Response
- SelectOfScalar
- Session
- SessionDep
- datetime
- EntryType
- ge
- le
- Query
- Response
- SelectOfScalar
- Session
- SessionDep
- ge
- le
- Query
- Response
- Session
- SessionDep
- ge
- le
- Query
- Response
- Session
- SessionDep
- ge
- le
- Query
- Response
- SelectOfScalar
- Session
- SessionDep
- SQLModel
- datetime
- SQLModel
- Session
- Session
- SQLModel
- String
- URL
- Bool
- MainActor
- String
- CGFloat
- Color
- View
- Bool
- CGFloat
- Color
- Content
- View
- CGFloat
- Color
- Content
- View
- Content
- Equatable
- View
- Content
- View
- CGFloat
- Color
- Content
- View
- Color
- String
- View
- Bool
- Color
- Double
- LinearGradient
- String
- CGFloat
- CGFloat
- Font
- String
- Binding
- Bool
- Date
- Decoder
- Double
- Int
- Date
- Int
- String
- String
- DateFormatter
- Int
- String
- Bool
- Int
- String
- Date
- Int
- UUID
- Bool
- Decoder
- Int
- Set
- UUID
- Date
- EntryType
- InitiatedBy
- Int
- UUID
- Bool
- Date
- Int
- AnalyticsOverview
- Date
- EmotionalLandscape
- GrowthIndicators
- Int
- SelfCareAnalytics
- TemporalPatterns
- Bool
- Data
- Date
- Error
- JSONDecoder
- JSONEncoder
- String
- URL
- Bool
- Date
- Int
- UUID
- Bool
- Date
- Int
- OpaquePointer
- String
- UUID
- Bool
- Error
- Int
- OpaquePointer
- String
- UUID
- Date
- Int
- UUID
- AnyCancellable
- Bool
- Date
- Double
- Error
- Int
- AnalyticsOverview
- Date
- Double
- EmotionalLandscape
- GrowthIndicators
- Int
- SelfCareAnalytics
- String
- TemporalPatterns
- AttributedString
- Result
- String
- Bool
- Published
- Bool
- AttributedString
- String
- AnalyticsOverview
- Int
- String
- EmotionalLandscape
- Error
- Int
- LayerDistributionItem
- PhaseDistributionItem
- TopEmotionItem
- Bool
- String
- Date
- Double
- GrowthIndicators
- Int
- String
- Int
- String
- AnyCancellable
- Int
- Set
- Binding
- Bool
- Int
- String
- Bool
- Double
- Int
- SelfCareAnalytics
- String
- TopStrategyItem
- Bool
- Date
- Int
- String
- TemporalPatterns
- String
- Date
- AnalyticsOverview
- String
- AnalyticsOverview
- Int
- String
- View
- Color
- String
- TopEmotionItem
- Bool
- Color
- GrowthIndicators
- String
- Date
- Int
- String
- Int
- String
- LayerDistributionItem
- String
- Color
- PhaseDistributionItem
- String
- Double
- Int
- SelfCareAnalytics
- String
- TopStrategyItem
- Color
- Date
- Double
- Int
- String
- TemporalPatterns
- CGFloat
- Color
- Font
- String
- String
- Void
- CGFloat
- Content
- CGFloat
- Color
- Double
- Font
- String
- String
- Bool
- Color
- CGFloat
- Color
- Double
- String
- Color
- String
- Bool
- Color
- Int
- String
- Color
- String
- Bool
- Color
- Color
- Int
- String
- Bool
- Color
- Int
- Color
- Int
- Binding
- Bool
- Content
- MainActor
- View
- Void
- Void
- Int
- String
- Void
- Double
- CGFloat
- Int
- Bool
- Content
- View
- CGFloat
- Int
- CGSize
- Double
- Int
- Bool
- CGSize
- Int
- LinearGradient
- Binding
- Bool
- Content
- View
- Content
- View
- Bool
- Void
- CGFloat
- Color
- Content
- Double
- String
- CGFloat
- Color
- ToolbarContent
- Bool
- String
- Date
- DateFormatter
- Int
- Set
- String
- Void
- Bool
- DateFormatter
- String
- Date
- Color
- String
- String
- InitiatedBy
- String
- Void
- Date
- Double
- Int
- String
- Void
- AnalyticsOverview
- Bool
- Date
- EmotionalLandscape
- GrowthIndicators
- Int
- SelfCareAnalytics
- TemporalPatterns
- UUID
- Date
- String
- String
- String
- Int
- String
- Int
- AnalyticsOverview
- Date
- EmotionalLandscape
- Error
- GrowthIndicators
- Int
- SelfCareAnalytics
- TemporalPatterns
- Any
- Date
- Encodable
- Error
- Int
- Response
- Result
- String
- T
- UUID
- Date
- Int
- UUID
- Int
- String
- UUID
- Bool
- Int
- String
- UUID
- Bool
- Encodable
- Error
- Int
- Published
- Response
- String
- T
- UUID
- Int
- Bool
- Encodable
- Error
- Int
- Response
- String
- T
- String
- Bool
- Double
- String
- Any
- Bool
- Data
- Encodable
- Error
- InitiatedBy
- Int
- JSONDecoder
- JSONEncoder
- Response
- Result
- Set
- String
- T
- Date
- Bool
- MonkeyPatch
- Path
- TestClient
- TestClient
- FastAPI
- MonkeyPatch
- Any
- datetime
- MonkeyPatch
- Path
- MainContentLifecycleModifier

## God Nodes (most connected - your core abstractions)
1. `LocalJournalEntry` - 143 edges
2. `SwiftUI` - 108 edges
3. `Foundation` - 80 edges
4. `SyncSettings` - 79 edges
5. `InMemoryJournalRepository` - 78 edges
6. `Testing` - 75 edges
7. `WavelengthWatch_Watch_App` - 75 edges
8. `LocalAnalyticsCalculator` - 69 edges
9. `JournalQueue` - 66 edges
10. `ContentViewModel` - 60 edges

## Surprising Connections (you probably didn't know these)
- `test_leaves_explicit_driver_and_sqlite_urls_untouched()` --calls--> `_normalize_database_url()`  [INFERRED]
  tests/backend/test_database_url.py → backend/database.py
- `test_normalizes_bare_postgres_schemes_to_psycopg()` --calls--> `_normalize_database_url()`  [INFERRED]
  tests/backend/test_database_url.py → backend/database.py
- `test_preserves_credentials_when_swapping_scheme()` --calls--> `_normalize_database_url()`  [INFERRED]
  tests/backend/test_database_url.py → backend/database.py
- `test_preserves_path_and_query_when_swapping_scheme()` --calls--> `_normalize_database_url()`  [INFERRED]
  tests/backend/test_database_url.py → backend/database.py
- `test_catalog_returns_empty_payload_when_database_cleared()` --indirect_call--> `Layer`  [INFERRED]
  tests/backend/test_catalog_api.py → backend/models.py

## Import Cycles
- None detected.

## Communities (652 total, 540 thin omitted)

### Community 1 - "Community 1"
Cohesion: 0.14
Nodes (7): UNAuthorizationOptions, UNNotificationCategory, UNUserNotificationCenter, NotificationCenterProtocol, NotificationScheduler, MockNotificationCenter, NotificationSchedulerTests

### Community 2 - "Community 2"
Cohesion: 0.07
Nodes (27): GrowthIndicatorsViewModelTests, MockAnalyticsService, MockJournalRepository, MockLocalAnalyticsCalculator, MockSyncSettingsPersistence, AnalyticsOverview, Bool, Date (+19 more)

### Community 5 - "Community 5"
Cohesion: 0.19
Nodes (8): FileManager, CatalogAPIService, CatalogCachePersisting, CatalogRemoteServicing, CatalogRepository, CatalogRepositoryLogging, DefaultCatalogRepositoryLogger, FileCatalogCacheStore

### Community 6 - "Community 6"
Cohesion: 0.09
Nodes (52): StrEnum, Dosage, EntryType, InitiatedBy, Dosage categories for curriculum entries., Source of journal entry creation., Type of journal entry.      Only ``EMOTION`` remains; the rest-period feature wa, AnalyticsOverview (+44 more)

### Community 7 - "Community 7"
Cohesion: 0.08
Nodes (10): Calendar, JournalEntryDrilldownFilter, byCurriculum, byHour, byLayer, byPhase, byStrategy, JournalEntryListView (+2 more)

### Community 8 - "Community 8"
Cohesion: 0.12
Nodes (30): alias, description, Header, get_session(), FastAPI dependency that yields a transactional session., IdempotencyRecord, Idempotency key tracking for journal creation.      Stores mapping of user-scope, _base_query() (+22 more)

### Community 9 - "Community 9"
Cohesion: 0.08
Nodes (24): MockAnalyticsService, MockJournalRepository, MockLocalAnalyticsCalculator, MockSyncSettingsPersistence, AnalyticsOverview, Bool, Date, Double (+16 more)

### Community 10 - "Community 10"
Cohesion: 0.10
Nodes (4): CustomStringConvertible, LocalJournalEntry, LocalAnalyticsCalculator, LocalAnalyticsCalculatorTests

### Community 11 - "Community 11"
Cohesion: 0.11
Nodes (3): SwiftUI, Testing, WavelengthWatch_Watch_App

### Community 12 - "Community 12"
Cohesion: 0.12
Nodes (4): PhaseNavigator, NavigationViewModel, NavigationViewModelTests, PhaseNavigatorTests

### Community 13 - "Community 13"
Cohesion: 0.12
Nodes (9): GrowthIndicators, DiversityCoverageView, GrowthIndicatorsView, MedicinalTrendView, TrendDirection, more, quieter, steady (+1 more)

### Community 14 - "Community 14"
Cohesion: 0.12
Nodes (4): Bool, Bool, layers, FlowCoordinator

### Community 15 - "Community 15"
Cohesion: 0.19
Nodes (7): ContentViewModel, InMemoryJournalRepository, ContentViewFilteringTests, ContentViewModelTests, CatalogRepositoryMock, JournalClientMock, ContentViewModelInitiationContextTests

### Community 16 - "Community 16"
Cohesion: 0.18
Nodes (6): TimeInterval, JournalSyncService, JournalSyncServiceTests, MockAPIClient, MockJournalQueue, MockNetworkMonitor

### Community 17 - "Community 17"
Cohesion: 0.06
Nodes (36): CodingKeys, avgFrequency, count, currentStreak, curriculumId, diversityScore, dominantDosage, dominantLayerId (+28 more)

### Community 18 - "Community 18"
Cohesion: 0.09
Nodes (32): make_cache_key(), Simple in-memory cache with TTL for analytics endpoints.  This module provides a, Generate a deterministic cache key for analytics queries.      The key format is, Curriculum, Journal, SQLModel table definitions for the WavelengthWatch backend., User journal entries representing runtime activity.      Performance note: The c, Curriculum entries that map layers to phases and expressions. (+24 more)

### Community 19 - "Community 19"
Cohesion: 0.16
Nodes (10): JournalFeedback, Kind, failure, queued, success, syncing, syncSuccess, InitiatedBy (+2 more)

### Community 20 - "Community 20"
Cohesion: 0.08
Nodes (16): JournalQueueError, databaseError, entryNotFound, insertFailed, invalidData, queryFailed, updateFailed, JournalQueueItem (+8 more)

### Community 21 - "Community 21"
Cohesion: 0.08
Nodes (23): MockAnalyticsService, MockJournalRepository, MockLocalAnalyticsCalculator, MockSyncSettingsPersistence, SelfCareViewModelTests, AnalyticsOverview, Bool, Date (+15 more)

### Community 22 - "Community 22"
Cohesion: 0.19
Nodes (6): APIClientError, Bool, Int, APIClientErrorRetryableTests, OfflineQueueIntegrationTests, ToggleableAPIClient

### Community 23 - "Community 23"
Cohesion: 0.26
Nodes (7): ContentView, JournalDrilldownContext, CatalogRepositoryProtocol, JournalClientProtocol, JournalRepositoryProtocol, AnalyticsDetailHubView, RootShellView

### Community 24 - "Community 24"
Cohesion: 0.10
Nodes (30): create_test_entries(), measure_response_time(), perf_client(), Performance tests for analytics endpoints.  Validates spec requirement: "Analyti, Test emotional landscape endpoint performance., Test self-care analytics endpoint performance., Test temporal patterns endpoint performance., Test growth indicators endpoint performance. (+22 more)

### Community 25 - "Community 25"
Cohesion: 0.22
Nodes (30): CompletedProcess, env(), _failing_pr(), _passing_pr(), _pending_pr(), End-to-end tests for ``scripts/pr-status.sh``.  The script shells out to ``gh pr, Isolate cache dir and ensure the script exists before each test., Per-test bundle of the tmp dir and the active ``monkeypatch``. (+22 more)

### Community 26 - "Community 26"
Cohesion: 0.16
Nodes (4): AnalyticsService, GrowthIndicatorsDetailView, SelfCareDetailView, TemporalPatternsDetailView

### Community 27 - "Community 27"
Cohesion: 0.09
Nodes (24): LogRecord, create_application(), Configure and return the FastAPI application., configure_logging(), _is_sensitive_field(), Logging configuration utilities that scrub PII before emission., Install the sensitive-data filter on known loggers., Apply lightweight regex scrubbing to formatted string messages. (+16 more)

### Community 28 - "Community 28"
Cohesion: 0.12
Nodes (14): Error, JournalError, queuedForRetry, JournalSyncStatus, error, idle, success, syncing (+6 more)

### Community 29 - "Community 29"
Cohesion: 0.17
Nodes (11): LogCaptureFixture, Sequence, Privacy-focused logging tests., Ensure each test starts with a clean root logger., reset_logging_filters(), _SequenceRequiringMarker, test_log_record_factory_invokes_filter(), test_logging_filter_handles_sequence_constructor_fallback() (+3 more)

### Community 30 - "Community 30"
Cohesion: 0.08
Nodes (23): AnalyticsViewModelTests, MockAnalyticsService, MockJournalRepository, MockLocalAnalyticsCalculator, AnalyticsOverview, Date, EmotionalLandscape, Error (+15 more)

### Community 31 - "Community 31"
Cohesion: 0.16
Nodes (6): HourlyDistributionItem, TemporalPatterns, HourlyRow, HourlySummary, TemporalPatternsView, TemporalPatternsViewTests

### Community 32 - "Community 32"
Cohesion: 0.10
Nodes (4): HeaderCapturingAPIClientSpy, InMemoryJournalQueueSpy, JournalClientQueueIntegrationTests, StubAPIClientSpy

### Community 33 - "Community 33"
Cohesion: 0.09
Nodes (25): RuntimeError, _determine_allowed_origins(), FastAPI application entrypoint., Return CORS origins based on the current environment configuration., get_catalog(), Router exposing the aggregated catalog endpoint., Return the cached catalog payload for clients.      The payload nests medicinal/, CatalogCurriculumEntry (+17 more)

### Community 34 - "Community 34"
Cohesion: 0.15
Nodes (3): AnimatedProgressPreview, CircularProgressView, CircularProgressViewTests

### Community 35 - "Community 35"
Cohesion: 0.14
Nodes (8): EmotionalLandscapeViewModel, LoadingState, error, idle, loaded, loading, EmotionalLandscapeViewModelTests, MockAnalyticsService

### Community 36 - "Community 36"
Cohesion: 0.18
Nodes (10): NSObject, UNNotification, UNNotificationPresentationOptions, UNNotificationRequest, UNNotificationResponse, UNUserNotificationCenterDelegate, NotificationDelegate, NotificationDelegateShim (+2 more)

### Community 37 - "Community 37"
Cohesion: 0.24
Nodes (9): Hashable, Identifiable, CatalogCurriculumEntryModel, CatalogDosage, medicinal, toxic, CatalogLayerModel, BarChartItem (+1 more)

### Community 38 - "Community 38"
Cohesion: 0.11
Nodes (14): DispatchQueue, Network, NWPath, NWPathMonitor, ObservableObject, ConnectionType, cellular, none (+6 more)

### Community 39 - "Community 39"
Cohesion: 0.09
Nodes (21): Always Use Test Scripts, Not Direct Tool Invocation, Architecture Overview, Backend (FastAPI), CI Requirements, Code Quality Standards, Command Execution Guidelines, Data Flow, Development Commands (+13 more)

### Community 40 - "Community 40"
Cohesion: 0.23
Nodes (4): CatalogStrategyModel, StrategyExpressionCard, StrategyListView, StrategyExpressionCardTests

### Community 41 - "Community 41"
Cohesion: 0.16
Nodes (5): ClearLightEmotionCard, CurriculumCard, LayeredEmotion, StrategyCard, CurriculumCardTests

### Community 42 - "Community 42"
Cohesion: 0.16
Nodes (18): Self-care strategies associated with specific layers and phases., Strategy, _iter_rows(), _load_curriculum(), _load_journal(), _load_layers(), _load_phases(), _load_strategies() (+10 more)

### Community 43 - "Community 43"
Cohesion: 0.11
Nodes (6): String, Color, ScheduleEditView, FailingAPIClientSpy, TrackingAPIClientSpy, APIClientSpy

### Community 44 - "Community 44"
Cohesion: 0.17
Nodes (4): MockCatalogRepository, Bool, CatalogResponseModel, ClearLightAggregationTests

### Community 45 - "Community 45"
Cohesion: 0.23
Nodes (9): PhaseStrategyGroup, SelfCareAnalytics, TopStrategyItem, OverallDiversityView, PhaseStrategyCardView, ResolvedPhaseGroup, StrategyCardView, StrategyUsageView (+1 more)

### Community 46 - "Community 46"
Cohesion: 0.10
Nodes (12): EntryType, emotion, InitiatedBy, scheduled, self_initiated, JournalClient, JournalPayload, JournalRequestHeader (+4 more)

### Community 48 - "Community 48"
Cohesion: 0.53
Nodes (8): Codable, Equatable, AnalyticsOverview, EmotionalLandscape, LayerDistributionItem, PhaseDistributionItem, PhaseMedicinalRatioItem, TopEmotionItem

### Community 49 - "Community 49"
Cohesion: 0.19
Nodes (6): LocalizedError, os, ContentViewDependencies, ContentViewDependenciesTests, FactoryFailure, ReasonedFailure

### Community 50 - "Community 50"
Cohesion: 0.08
Nodes (15): View, colorSwatch(), AnalyticsView, EmotionRow, EmotionSection, JournalEntryRowView, MarkdownContentView, MenuView (+7 more)

### Community 52 - "Community 52"
Cohesion: 0.10
Nodes (5): OnboardingViewUITests, WavelengthWatch_Watch_AppUITests, WavelengthWatch_Watch_AppUITestsLaunchTests, XCTest, XCTestCase

### Community 53 - "Community 53"
Cohesion: 0.15
Nodes (17): Engine, configure_engine(), create_db_and_tables(), _create_engine(), _is_sqlite(), _normalize_database_url(), Database configuration and session utilities., Pin the psycopg driver on bare Postgres URLs; pass others through.      Schemes (+9 more)

### Community 54 - "Community 54"
Cohesion: 0.17
Nodes (11): CaseIterable, FlowStep, confirmingPrimary, confirmingSecondary, confirmingStrategy, idle, review, selectingPrimary (+3 more)

### Community 55 - "Community 55"
Cohesion: 0.22
Nodes (3): BarRow, HorizontalBarChart, HorizontalBarChartTests

### Community 57 - "Community 57"
Cohesion: 0.18
Nodes (14): APIClient, badResponse, invalidURL, transport, Data, Encodable, JSONDecoder, JSONEncoder (+6 more)

### Community 58 - "Community 58"
Cohesion: 0.17
Nodes (6): ButtonStyle, Configuration, View, WLPrimaryButtonStyle, WLSecondaryButtonStyle, WLButtonStyleTests

### Community 59 - "Community 59"
Cohesion: 0.18
Nodes (7): DateComponents, Encoder, JournalSchedule, UserDefaults, JournalScheduleTests, ScheduleViewModelTests, JournalClientTests

### Community 60 - "Community 60"
Cohesion: 0.12
Nodes (10): AnalyticsCache, CacheEntry, Clear all cache entries.          Useful for testing or when a full cache invali, Return cache statistics for monitoring.          Returns:             Dictionary, A cached value with expiration timestamp., Thread-safe in-memory cache with TTL support.      Optimized for analytics endpo, Initialize cache with specified TTL.          Args:             ttl_seconds: Tim, Get a value from cache if it exists and hasn't expired.          Args: (+2 more)

### Community 61 - "Community 61"
Cohesion: 0.22
Nodes (6): Bundle, OSLog, AppConfiguration, BundleProtocol, AppConfigurationTests, MockBundle

### Community 62 - "Community 62"
Cohesion: 0.12
Nodes (10): JournalDatabase, JournalDatabaseError, databaseNotOpen, failedToCreateTable, failedToDelete, failedToInsert, failedToOpenDatabase, failedToQuery (+2 more)

### Community 64 - "Community 64"
Cohesion: 0.12
Nodes (13): Tests for journal endpoints., Test creating journal entry without idempotency key (current behavior)., Test creating journal entry with new idempotency key creates entry., Test same idempotency key twice returns existing entry, no duplicate., Test idempotency key must be valid UUID format., Test same idempotency key used by different users creates separate entries., Test initiated_by field validation and defaults., test_journal_create_with_new_idempotency_key() (+5 more)

### Community 65 - "Community 65"
Cohesion: 0.13
Nodes (14): Configuration, Curriculum, Deployment (Railway), Example Requests, Health Check, Installation, Journal Entries, Layers (+6 more)

### Community 66 - "Community 66"
Cohesion: 0.18
Nodes (6): View, WLGlassIntensity, prominent, regular, WLGlassModifier, WLGlassModifierTests

### Community 68 - "Community 68"
Cohesion: 0.33
Nodes (4): RootPresentationHost, Content, View, View

### Community 69 - "Community 69"
Cohesion: 0.23
Nodes (3): CatalogPhaseModel, PhaseJourneyView, PhaseJourneyViewTests

### Community 70 - "Community 70"
Cohesion: 0.42
Nodes (3): CatalogCacheEnvelope, CatalogRemoteStub, CatalogRepositoryTests

### Community 71 - "Community 71"
Cohesion: 0.19
Nodes (4): Color, ModeDistributionView, LayerSideIndicator, ModeDistributionViewTests

### Community 72 - "Community 72"
Cohesion: 0.15
Nodes (12): 1. **Run the Setup Script**, 2. **Start the Backend Server**, 3. **Build and Run the Watch App**, Configuration Priority, Local Development Setup, Manual Setup, Quick Setup, Reset Configuration (+4 more)

### Community 73 - "Community 73"
Cohesion: 0.28
Nodes (6): PreviewProvider, OnboardingView, OnboardingView_Previews, StorageMode, cloudSynced, localOnly

### Community 75 - "Community 75"
Cohesion: 0.31
Nodes (12): Layer, Reference table describing each spiral dynamics layer., create_layer(), delete_layer(), get_layer(), _get_layer_or_404(), list_layers(), Layer CRUD endpoints. (+4 more)

### Community 76 - "Community 76"
Cohesion: 0.22
Nodes (4): GeometryProxy, LayerCardView, LayerScrollView, LayerCardViewTests

### Community 77 - "Community 77"
Cohesion: 0.26
Nodes (3): Alert, JournalFeedbackAlert, JournalFeedbackAlertTests

### Community 78 - "Community 78"
Cohesion: 0.31
Nodes (5): AnyShapeStyle, ShapeStyle, View, WLCardSurface, WLSurfaceModifierTests

### Community 79 - "Community 79"
Cohesion: 0.12
Nodes (9): MarkdownContent, MarkdownContentLoader, MarkdownLoadError, fileNotFound, parsingFailed, readFailed, ConceptExplainerView, ConceptExplainerViewTests (+1 more)

### Community 80 - "Community 80"
Cohesion: 0.38
Nodes (12): _base_query(), create_curriculum(), delete_curriculum(), _ensure_curriculum_id(), get_curriculum(), _get_curriculum_or_404(), list_curriculum(), Curriculum endpoints. (+4 more)

### Community 81 - "Community 81"
Cohesion: 0.38
Nodes (12): _base_query(), create_strategy(), delete_strategy(), _ensure_strategy_id(), get_strategy(), _get_strategy_or_404(), list_strategies(), _serialize_strategy() (+4 more)

### Community 82 - "Community 82"
Cohesion: 0.22
Nodes (8): CodingKey, CodingKeys, phaseOrder, CodingKeys, enabled, id, repeatDays, time

### Community 83 - "Community 83"
Cohesion: 0.23
Nodes (6): FlowStepReaction, FlowStepReactionPolicy, ReviewSheetAction, dismissIfActive, present, FlowStepReactionPolicyTests

### Community 84 - "Community 84"
Cohesion: 0.18
Nodes (8): ActivePresentation, flowReview, idle, journalFeedback, logConfirmation, menu, onboarding, storageError

### Community 85 - "Community 85"
Cohesion: 0.24
Nodes (7): ci_human_line(), die_usage(), render_default(), render_summary(), pr-status.sh script, update_overall(), usage()

### Community 86 - "Community 86"
Cohesion: 0.24
Nodes (4): IndexSet, NotificationSchedulerProtocol, ScheduleViewModel, ScheduleSettingsView

### Community 87 - "Community 87"
Cohesion: 0.13
Nodes (10): WatchOSMarkdownParserTests, MarkdownBlock, blockquote, empty, header1, header2, header3, listItem (+2 more)

### Community 88 - "Community 88"
Cohesion: 0.35
Nodes (11): Phase, Reference table of user energy phases., create_phase(), delete_phase(), get_phase(), _get_phase_or_404(), list_phases(), Phase CRUD endpoints. (+3 more)

### Community 89 - "Community 89"
Cohesion: 0.24
Nodes (5): DetailDestination, curriculum, strategy, DetailDestinationView, DetailDestinationTests

### Community 91 - "Community 91"
Cohesion: 0.17
Nodes (11): Tests for analytics endpoints., Test dominant phase/dosage with mixed entries across hours., Test strategies grouped by phase in self-care analytics., Test analytics for user with no entries., Test that top emotions includes secondary emotions., Test basic analytics overview with default date range., test_analytics_overview_basic(), test_analytics_overview_empty_user() (+3 more)

### Community 92 - "Community 92"
Cohesion: 0.18
Nodes (10): Backend (FastAPI), Configuring the Watch API Base URL, Frontend (watchOS), Getting Started, Implementation Snapshot, Knowledge graph, Overview, Repository Structure (+2 more)

### Community 93 - "Community 93"
Cohesion: 0.24
Nodes (5): Sendable, SubmitTimeout, SubmitTimeoutError, ProbeError, SubmitTimeoutTests

### Community 94 - "Community 94"
Cohesion: 0.11
Nodes (5): UIConstants, WLSpacingTokens, WLTheme, WLTypographyTokens, EnvironmentValues

### Community 95 - "Community 95"
Cohesion: 0.25
Nodes (3): View, WLCardModifier, WLCardModifierTests

### Community 97 - "Community 97"
Cohesion: 0.24
Nodes (5): AnalyticsEmptyView, Prominence, compact, prominent, AnalyticsEmptyViewTests

### Community 98 - "Community 98"
Cohesion: 0.38
Nodes (4): Animation, V, View, WLConditionalAnimation

### Community 100 - "Community 100"
Cohesion: 0.33
Nodes (4): Action, curriculum, strategy, LogConfirmationRequest

### Community 104 - "Community 104"
Cohesion: 0.13
Nodes (8): ViewModifier, FlowConfirmationAlertsModifier, View, JournalFlowAlertsModifier, View, CardDepthShadow, CardIntensityLadder, PhaseCrystalCard

### Community 107 - "Community 107"
Cohesion: 0.26
Nodes (4): CatalogRepositoryFallbackTests, NetworkDown, FailingRemoteStub, InMemoryCatalogCacheMock

### Community 109 - "Community 109"
Cohesion: 0.20
Nodes (9): Tests for journal entry_type behavior in the journal API., EMOTION entries work normally with curriculum_id., If entry_type is not specified, it defaults to EMOTION., The removed 'rest' entry_type is no longer accepted (#435)., EMOTION entries must have curriculum_id., test_create_emotion_entry_requires_curriculum(), test_create_emotion_entry_with_curriculum(), test_entry_type_defaults_to_emotion() (+1 more)

### Community 111 - "Community 111"
Cohesion: 0.28
Nodes (3): View, WLNavigationBarModifier, WLNavigationBarModifierTests

### Community 112 - "Community 112"
Cohesion: 0.22
Nodes (9): CodingKeys, createdAt, curriculumID, entryType, id, initiatedBy, secondaryCurriculumID, strategyID (+1 more)

### Community 113 - "Community 113"
Cohesion: 0.38
Nodes (3): Binding, Bool, MainContentDialogsModifierTests

### Community 117 - "Community 117"
Cohesion: 0.08
Nodes (4): Combine, Foundation, SQLite3, AppStorageKeys

### Community 121 - "Community 121"
Cohesion: 0.25
Nodes (7): Tests for database startup and schema validation., Regression test for ensuring database startup works with correct Strategy schema, Direct database test to ensure Strategy model works correctly.      This test by, Test that the FastAPI app startup lifecycle completes without errors.      This, test_app_startup_completes_successfully(), test_database_direct_strategy_query(), test_database_startup_creates_correct_schema()

### Community 123 - "Community 123"
Cohesion: 0.40
Nodes (3): ClosedRange, Self, Comparable

### Community 124 - "Community 124"
Cohesion: 0.33
Nodes (3): Never, Task, FlowReviewSheet

### Community 126 - "Community 126"
Cohesion: 0.40
Nodes (4): LayerView, CGFloat, Double, Int

### Community 131 - "Community 131"
Cohesion: 0.50
Nodes (3): App, Scene, WavelengthWatch_Watch_AppApp

### Community 132 - "Community 132"
Cohesion: 0.33
Nodes (5): Archetypal Wavelength, Dosages, Frequencies, Phases, Self-Care Strategies

### Community 134 - "Community 134"
Cohesion: 0.60
Nodes (5): _convert_curriculum(), _convert_headers(), _convert_strategies(), _detect(), main()

### Community 135 - "Community 135"
Cohesion: 0.25
Nodes (4): LayerFilterMode, all, emotionsOnly, strategiesOnly

### Community 136 - "Community 136"
Cohesion: 0.40
Nodes (3): APIClientProtocol, APIPath, JournalQueueProtocol

### Community 137 - "Community 137"
Cohesion: 0.40
Nodes (3): AnalyticsLoadingView, ShimmerModifier, View

### Community 140 - "Community 140"
Cohesion: 0.40
Nodes (4): Contributing to WavelengthWatch, Journal System Reference, Pull Request Checklist, Workflow Expectations

### Community 141 - "Community 141"
Cohesion: 0.40
Nodes (4): SyncStatus, failed, pending, synced

### Community 144 - "Community 144"
Cohesion: 0.50
Nodes (3): ⚙️ Agent Behavior and Development Philosophy, Journal Implementation Snapshot, Repository Directory Overview

## Knowledge Gaps
- **227 isolated node(s):** `dev-setup.sh script`, `AppStorageKeys`, `os`, `regular`, `prominent` (+222 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **540 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Foundation` connect `Community 117` to `Community 1`, `Community 2`, `Community 3`, `Community 5`, `Community 7`, `Community 136`, `Community 9`, `Community 135`, `Community 11`, `Community 12`, `Community 141`, `Community 16`, `Community 145`, `Community 20`, `Community 21`, `Community 23`, `Community 30`, `Community 35`, `Community 37`, `Community 38`, `Community 46`, `Community 48`, `Community 49`, `Community 51`, `Community 54`, `Community 61`, `Community 74`, `Community 82`, `Community 83`, `Community 93`, `Community 100`, `Community 106`, `Community 116`, `Community 122`?**
  _High betweenness centrality (0.051) - this node is a cross-community bridge._
- **Why does `SwiftUI` connect `Community 11` to `Community 128`, `Community 7`, `Community 137`, `MainContentLifecycleModifier`, `Community 139`, `Community 13`, `Community 142`, `Community 143`, `Community 146`, `Community 23`, `Community 26`, `Community 31`, `Community 34`, `Community 36`, `Community 40`, `Community 41`, `Community 43`, `Community 45`, `Community 50`, `Community 55`, `Community 58`, `Community 63`, `Community 66`, `Community 67`, `Community 68`, `Community 69`, `Community 71`, `Community 73`, `Community 76`, `Community 77`, `Community 78`, `Community 79`, `Community 84`, `Community 86`, `Community 89`, `Community 90`, `Community 94`, `Community 95`, `Community 97`, `Community 98`, `Community 103`, `Community 104`, `Community 106`, `Community 111`, `Community 115`, `Community 117`, `Community 119`, `Community 124`, `Community 126`, `Community 127`?**
  _High betweenness centrality (0.049) - this node is a cross-community bridge._
- **Why does `LocalJournalEntry` connect `Community 10` to `Community 0`, `Community 2`, `Community 3`, `Community 37`, `Community 9`, `Community 43`, `Community 141`, `Community 48`, `Community 21`, `Community 118`, `Community 22`, `Community 153`, `Community 30`?**
  _High betweenness centrality (0.044) - this node is a cross-community bridge._
- **Are the 71 inferred relationships involving `LocalJournalEntry` (e.g. with `.fetchAll_tolerantOfInvalidSyncStatus()` and `.inMemoryClearRemovesAllEntries()`) actually correct?**
  _`LocalJournalEntry` has 71 INFERRED edges - model-reasoned connections that need verification._
- **What connects `dev-setup.sh script`, `AppStorageKeys`, `os` to the rest of the system?**
  _227 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Community 0` be split into smaller, more focused modules?**
  _Cohesion score 0.10869565217391304 - nodes in this community are weakly interconnected._
- **Should `Community 1` be split into smaller, more focused modules?**
  _Cohesion score 0.14492753623188406 - nodes in this community are weakly interconnected._