...
if [ "$INDIVIDUAL_MODE" = true ] || [ ${#SUITES[@]} -eq 1 ]; then
  ...
  for suite in "${SUITES[@]}"; do
    ...
    if ! TEST_OUTPUT=$(xcodebuild test-without-building \
      ...
      echo "---"
      echo "$TEST_OUTPUT" | grep -E -A 5 "failed on|Expectation failed|recorded an issue|XCTAssert.*failed|TEST FAILED|error: " | head -40 || echo "No error details found"
      echo "---"
  ...
  if ! TEST_OUTPUT=$(xcodebuild test-without-building \
    ...
    echo "---"
    echo "$TEST_OUTPUT" | grep -E -A 5 "failed on|Expectation failed|recorded an issue|XCTAssert.*failed|TEST FAILED|error: " | head -60 || echo "No error details found"
    echo "---"
